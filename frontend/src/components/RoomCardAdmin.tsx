...
type RoomCardAdminProps = RuanganProps & {
...
function RoomCardAdmin( props: RoomCardAdminProps) {
  ...
  const fasilitasToRender = useMemo(() => {
    return (fasilitasList ?? ruanganFasiltias).filter(item => item.id_ruangan === id_ruangan);
      ...
      <div className='flex flex-col gap-10'>
        ...
        <div className='flex flex-col'>
          ...
          <div className='flex flex-row justify-start gap-3 flex-wrap min-h-16'>
            { (fasilitasLoading || (shouldFetch && ruanganFasiltias.length === 0)) && (
              <>
            ...
            { fasilitasToRender.map((item) => (
                <Tag title={item.fasilitas.nama_fasilitas} key={`${item.id_ruangan}-${item.id_fasilitas}`} classname='px-10 mt-3 text-xl bg-[var(--gray-color)]'/>
            ))}
          </div>
        </div>
      </div>
      <div className='flex flex-row gap-5 text-2xl'>
        <Button classname='py-2 flex-3/8 bg-(--button-gray) border-[#CBCBCB] border-2 text-black hover:text-white' title='Detail' />
        <Button classname='py-2 flex-3/8 bg-(--button-gray) border-[#CBCBCB] border-2 text-black hover:text-white' title='Edit' />
