...
const Login: React.FC = () => {
  ...
  const [loginFailed, setLoginFailed] = useState<{ message?: string } | null>(
    null,
  );
  ...
  return (
    <div className="flex flex-col w-full h-screen">
      <header className="flex w-full lg:shadow-(--banner-shadow-lg) md:shadow-(--banner-shadow-md) shadow-(--banner-shadow) justify-center z-10">
        <img
          src={assets.bannerUpnvj}
          alt="banner-upnvj.png"
          loading="lazy"
      ...
      <main className="flex lg:flex-row md:flex-col flex-col w-full lg:h-screen">
        <div className="flex flex-col flex-1 gap-5 justify-center items-center p-10">
          ...
          <p className="lg:text-5xl md:text-3xl text-2xl font-medium">
            Selamat Datang di Sistem Informasi Peminjaman Sarana dan Prasarana
          </p>
        </div>
        <div className="w-full h-full flex flex-1 flex-col lg:gap-10 md:gap-7 gap-5 justify-center items-center lg:p-10 md:p-7 p-5">
          <div className="flex flex-col shadow-(--card-shadow) lg:p-10 md:p-7 p-4 lg:min-w-2xl md:min-w-xl lg:rounded-lg md:rounded-md rounded-sm">
            <form
              onSubmit={login}
              className="flex flex-col lg:gap-6 md:gap-5 gap-3"
            >
              <label className="lg:text-2xl md:text-xl text-md text-left text-gray-400">
                Masukan username dan password UPNVJ anda:
              </label>
              ...
              {loginFailed?.message && <Alerts>{loginFailed?.message}</Alerts>}
              <input
                type="text"
              ...
              <label className="lg:text-2xl md:text-xl text-sm flex flex-row lg:gap-3 md:gap-2 gap-1 items-center hover:cursor-pointer">
                <input
                  type="checkbox"
                  className="hover:cursor-pointer lg:w-7 lg:h-7 md:w-5 md:h-5 w-3 h-3 duration-300"
              ...
              <button
                type="submit"
                className="lg:text-3xl md:text-2xl text-lg font-medium text-white lg:px-8 lg:py-5 md:px-6 md:py-4 px-4 py-2 lg:rounded-lg md:rounded-md rounded-sm bg-(--blue-button) hover:cursor-pointer hover:bg-(--blue-button-hover) duration-300"
              >
                Login
              </button>
            </form>
            <div className="w-full ring-gray-200 ring-1 lg:mt-10 md:mt-7 mt-5"></div>
            <a className="lg:text-2xl md:text-xl text-md text-(--link-color) hover:text-(--link-hover-color) hover:cursor-pointer lg:mt-8 md:mt-6 mt-4">
              bantuan<i>(Help)</i>
            </a>
          </div>
          <div>
            <button
              ...
              className="lg:text-3xl md:text-2xl text-lg text-white lg:px-8 lg:py-5 md:px-6 md:py-4 px-4 py-2 lg:rounded-lg md:rounded-md rounded-sm bg-(--red-button) hover:bg-(--red-button-hover) hover:cursor-pointer duration-300"
            >
